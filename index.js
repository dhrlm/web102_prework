...
function addGamesToPage(games) {
    ...
    for (let i = 0; i < games.length; i++) {
        ...
        gamesCard.innerHTML = `<img class="game-img" src="${games[i].img}" />
        <h4>${games[i].name}</h4>
...
const displayStr = `A total of $${totalRaised.toLocaleString()} has been raised for ${totalGames} games. Currently, ${unfundedGamesCount === 1 ? '1 game remains unfunded.' : `${unfundedGamesCount} games remain unfunded.`} We need your help to fund these amazing games!`;

// create a new DOM element containing the template string and append it to the description container
const displayEle = document.createElement("p");
displayEle.textContent = displayStr;
...
function showFirstAndSecondGames() {
    ...
    const topAndRunner = [sortedGames[0], sortedGames[1]];
    addGamesToPage(topAndRunner);
}

const topAndRunnerBtn = document.getElementById("topandrunner-btn");
topAndRunnerBtn.addEventListener("click", showFirstAndSecondGames);